...
#                 print(gb)
                ...
                while kk < len(gb):
                    ...
                    kk+=1
                out_grids[g]=[obs, len(obs)]
        print(f"{t}:调出{len(out_grids)},调入{len(in_grids)}")
        return in_grids, out_grids
...
for t in range(DAY*24):
    ...
    if t % 168==0 and t > 0:
        ...
        save_data(f'../raw/{city}_round_{times}_3',x,y)
...
x.sort()
y = [result[_x] / len(bike_list22.values()) for _x in x]
save_data(f'../raw/{city}_round_{times+1}_3',x,y)
...
df.to_csv('beijing.csv',index=False)


# In[80]:


t=df[df['count']>1].groupby('t')['count'].sum().sort_values()
...
t.to_csv('bj_210.csv',index=False)
...
select_df.to_csv('bj_sim.csv',index=False)
