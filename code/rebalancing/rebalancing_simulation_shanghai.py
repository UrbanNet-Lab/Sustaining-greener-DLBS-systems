...
JH_TF=LS_TF
JH_ACC = 1000
JH_DIST = '../jinhua2.geojson'
JH_ST='2020-09-14 00:00:00.0'
...
# plt.hist(demand,len(demand),edgecolor='black')


# In[16]:


data = pd.DataFrame(bike0.order_data[1:])
...
data.columns = bike0.order_data[0]
data['end_time'] = pd.datetime(data['end_time'])
data['duration'] = data['end_time'] - data['start_time']
...
#         time.sleep(60)
    bike0.simulation(t, rank_list22, bike_list22,bike_trip22
...
#     print((rs[i] * max_demand)) 
    for thre in range(1, 2):
        ...
        for t in range(DAY*24):
            ...
            print(t)
            if True:
                bike2.moveBytimes(t,move_list22,thre,CENTRE_MAP,0.01*ACCURACY/1000,RebalancingList)
...
axes[0].set_ylabel('mean_demand', fontsize=14, family='Arial')
axes[1].plot(cv)
...
df = pd.DataFrame(RebalancingList)
df.columns = ['Start','Start_x','Start_y','End','End_x','End_y','count','t']
...
df=df[df['count']>1]
df.groupby('t')['count'].sum().sort_values()
t_584 = df[df['t']==584]
...
t_584.to_csv('shanghai_584.csv',index=False)
...
df_sim.dropna()
selected_columns = ['bikeid','sim_bike', 'start_time','end_time']
select_df = df_sim[selected_columns]
select_df.to_csv('sh_sim.csv',index=False)
