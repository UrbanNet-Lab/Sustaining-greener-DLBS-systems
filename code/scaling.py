...
def rank(bikeid, endx, endy, step):
    middle = [119.8380914813, 28.4436707765]
    dict2 = {}
    for i, j, k in zip(bikeid, endx, endy):
        if (np.isnan(i) == False):
            if i not in dict2.keys():
...
# data = pd.read_csv('jinhua-v1.csv')
# time1 = pd.to_datetime(data['StartTime'])
# endx = data['EndLng']
# endy = data['EndLat']
data2 = pd.read_csv('simulation/jinhua/simulation5.csv')
endx = data2['EndLng']
endy = data2['EndLat']
...
dict_all6 = {}
dict_all7 = {}
rank_x,rank_y = rank(data2['BikeID'],endx,endy,1)
# print(rank_x,rank_y)
