...
#deadline1=pd.datetime(2019, 10,  9, 0, 0)   #星期三
#deadline2=pd.datetime(2019, 10, 10, 0, 0)
...
#data_ha=data_new[['bikeid','starttime','endtime','startpositionx','startpositiony',
#                  'endpositionx','endpositiony','d']]
#data_ha.to_csv('C:/python/Nanjing2/orange_Oct19_NJ33_day1.csv',index=False)


nodes=10


#data=pd.read_csv('C:/python/Nanjing2/dyn/dyn_nj_250.csv') 
#rows=data.index
#rank_all=[]
#for row in rows:
...
#    degree[i[0]][i[1]]=j
#np.savetxt('C:/python/Nanjing2/dyn/dyn_nj_degree_%d_250.txt'%nodes,degree,fmt='%d',delimiter=',')
...
degree_bj=np.loadtxt('C:/python/Nanjing2/dyn/dyn_nj_degree_%d.txt'%nodes,dtype=int,delimiter=',')
degree_no=[]
degree_no_index=[]
degree_no_se=[]
for i in range(nodes):
...
#                              lw=degree_bj[i][j]/degree_max*line_wid_max,color=colors[0],alpha=degree_bj[i][j]/degree_max)
        if(i==j):
            ax.plot(c_dict[i][0],c_dict[i][1],marker='o',markersize=40,c=colors[0],
                    linewidth=0,alpha=degree_bj[i][j]/degree_max_se)      #60
...
plt.axis('off')
#plt.savefig('C:/python/摩拜单车/draw2/dyn_c.pdf',bbox_inches='tight') 
...
#
##degree_bj=np.loadtxt('C:/python/Nanjing2/dyn/dyn_nj_degree_%d.txt'%nodes,dtype=int,delimiter=',')
#degree_bj=np.loadtxt('C:/python/Nanjing2/dyn/dyn_nj_degree_%d_250.txt'%nodes,dtype=int,delimiter=',')
#degree_no=[]
#degree_no_se=[]
#for i in range(nodes):
#    for j in range(nodes):
...
#data=pd.read_csv('C:/python/Nanjing2/orange_Oct19_NJ33_day1.csv') 
#rows=data.index
#
#DR1=0.0025                                       
#bike_all=[]
...
#dict_init2={}
#dict_init3={}
#time_init=pd.to_datetime('2019/10/08 23:30:00')
#for i in bike_all:  
...
#data_new.to_csv('C:/python/Nanjing2/dyn/dyn_nj_250.csv',index=False)
