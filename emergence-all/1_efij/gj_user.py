...
#plt.savefig('1_j.pdf',bbox_inches='tight')
...
ax.set_ylabel("latitude",fontdict={ 'size'   : 18,"family":"Arial"})#纬度
#ax.scatter(cx,cy,color="red",s=30)
my_x_ticks = np.arange(115.3, 116.6, 0.2)
plt.xticks(my_x_ticks)
my_y_ticks = np.arange(39.4, 41.2, 0.2)
plt.yticks(my_y_ticks)
plt.xlim(116.175,116.7)
plt.ylim(39.7875,40.2125)
plt.yticks(fontproperties = 'Arial')
plt.xticks(fontproperties = 'Arial')
...
for i in range(len(path)):#某个用户
    for j in range(len(path[i])):
        ...
        plt.plot(lat,lon,alpha=0.5,color=colo,linewidth=2)
ax.scatter(cx,cy,color="red",s=30)
plt.text((116.7-116.175)*0.05+116.175,(40.2125-39.7875)*0.9+39.7875, 'Beijing',fontdict=dict(fontsize=18,family='Arial'))
#plt.savefig('1_i.pdf',bbox_inches='tight')
