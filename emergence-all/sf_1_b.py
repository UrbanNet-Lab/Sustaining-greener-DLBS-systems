...
x_xm=[0.5, 1.5, 2.5, 3.5, 4.5, 5.5, 6.5, 7.5, 8.5, 9.5, 10.5, 11.5, 12.5, 13.5]
y_xm=[0.6671790198185453, 0.2507447257096998, 0.05732460972745533, 0.016386232662740138, 0.005041917742381581,
      0.00199500342324451, 0.0007571944810950755, 0.0003083187108650607, 0.0001722957501892986, 
      1.8136394756768276e-05, 4.987508558111276e-05, 9.068197378384138e-06, 9.068197378384138e-06, 
      4.534098689192069e-06]
...
data7=pd.read_csv('C:/python/摩拜单车/距离/dis_sh_eu.txt')
data8=pd.read_csv('C:/python/摩拜单车/距离/dis_bj_eu.txt')
data1=pd.read_csv('C:/python/摩拜单车/距离/dis_nj.txt')
data2=pd.read_csv('C:/python/摩拜单车/距离/dis_nj_mo.txt')
data3=pd.read_csv('C:/python/摩拜单车/距离/dis_cd.txt')
data4=pd.read_csv('C:/python/摩拜单车/距离/dis_xa.txt')
data5=pd.read_csv('C:/python/摩拜单车/距离/dis_xm.txt')
data6=pd.read_csv('C:/python/摩拜单车/距离/dis_sgp.txt')

x_sh_eu,y_sh_eu=data7['x'],data7['y']
x_bj_eu,y_bj_eu=data8['x'],data8['y']
x_nj,y_nj=data1['x'],data1['y']
x_nj=list(x_nj)[1:]
y_nj=list(y_nj)[1:]
x_nj_mo,y_nj_mo=data2['x'],data2['y']
x_cd,y_cd=data3['x'],data3['y']
x_xa,y_xa=data4['x'],data4['y']
x_xm,y_xm=data5['x'],data5['y']
x_sgp,y_sgp=data6['x'],data6['y']

dict_all1={}
#for kk in [[x_sh_eu,y_sh_eu],[x_nj,y_nj],[x_nj_mo,y_nj_mo],[x_cd,y_cd],[x_xa,y_xa],[x_xm,y_xm],[x_sgp,y_sgp]]:
for kk in [[x_sh_eu,y_sh_eu],[x_nj,y_nj],[x_nj_mo,y_nj_mo],[x_cd,y_cd],[x_xa,y_xa],[x_xm,y_xm]]:
    for i,j in zip(kk[0],kk[1]):
...
#        all1.append(np.sum(value)/6)
        all1_x.append(key)
all1_x,all1 = (list(t) for t in zip(*sorted(zip(all1_x,all1)))) 
  
x_sh_eu,y_sh_eu=log_pdf(x_sh_eu[:],y_sh_eu[:],0,3,35)
x_bj_eu,y_bj_eu=log_pdf(x_bj_eu[:],y_bj_eu[:],0,3,30) 
x_nj,y_nj=log_pdf(x_nj,y_nj,0,3,35)
x_nj_mo,y_nj_mo=log_pdf(x_nj_mo,y_nj_mo,0,3,35)  
x_cd,y_cd=log_pdf(x_cd,y_cd,0,3,35)
x_xa,y_xa=log_pdf(x_xa,y_xa,0,3,35) 
x_xm,y_xm=log_pdf(x_xm,y_xm,0,3,35)
x_sgp,y_sgp=log_pdf(x_sgp,y_sgp,0,3,35)
  
    
#cut=8   #5,8
#popt, pcov = curve_fit(func4, x_nj_mo[cut:],y_nj_mo[cut:],maxfev = 10000)   
#y777 = [func4(i, *popt) for i in x_nj_mo]
#ax.plot(x_nj_mo,y_nj_mo,marker='s',markersize=10,markerfacecolor='none',markeredgewidth=2,linewidth=0,label=r'D4, $\mu$=%d, $\sigma$=%d'%(popt[1]*100,popt[2]*100))
#ax.plot(x_nj_mo,y777,'b-')




ax.plot(x_sh_eu,y_sh_eu,marker='s',markersize=10,markerfacecolor='none',markeredgewidth=2,linewidth=0,label=r'D1 (SH)')
ax.plot(x_bj_eu,y_bj_eu,marker='s',markersize=10,markerfacecolor='none',markeredgewidth=2,linewidth=0,label=r'D2 (BJ)')

ax.plot(x_nj,y_nj,marker='s',markersize=10,markerfacecolor='none',markeredgewidth=2,linewidth=0,label=r'D3 (NJ)')
#popt, pcov = curve_fit(func4, x_nj[2:],y_nj[2:],maxfev = 10000)
#y999 = [func4(i, *popt) for i in x_nj]
#ax.plot(x_nj,y_nj,marker='s',markersize=10,markerfacecolor='none',markeredgewidth=2,linewidth=0,label=r'D3, $\mu$=%d, $\sigma$=%d'%(popt[1]*100,popt[2]*100))

#cut=10
#popt, pcov = curve_fit(func2, x_nj[cut:],y_nj[cut:],maxfev = 10000)
#y999 = [func2(i, *popt) for i in x_nj]
#ax.plot(x_nj,y_nj,marker='s',markersize=10,markerfacecolor='none',markeredgewidth=2,linewidth=0,label=r'D3, $\alpha$=%.2f'%popt[1])

ax.plot(x_nj_mo,y_nj_mo,marker='s',markersize=10,markerfacecolor='none',markeredgewidth=2,linewidth=0,label=r'D4 (NJ)')
ax.plot(x_cd,y_cd,marker='s',markersize=10,markerfacecolor='none',markeredgewidth=2,linewidth=0,label=r'D5 (CD)')
ax.plot(x_xa,y_xa,marker='s',markersize=10,markerfacecolor='none',markeredgewidth=2,linewidth=0,label=r'D6 (XA)')
ax.plot(x_xm,y_xm,marker='s',markersize=10,markerfacecolor='none',markeredgewidth=2,linewidth=0,label=r'D7 (XM)')
ax.plot(x_sgp,y_sgp,marker='s',markersize=10,markerfacecolor='none',markeredgewidth=2,linewidth=0,label=r'D8 (SGP)')

#ax.plot(x_nj,y999,color='g',linestyle='-')
cut=17  #11,15,17
popt, pcov = curve_fit(func2, all1_x[cut:],all1[cut:],maxfev = 10000)
y999 = [func2(i, *popt) for i in all1_x]
#ax.plot(all1_x,all1,linestyle='-',label=r'allll')
ax.plot(all1_x,y999,c='k',linestyle='-',linewidth=3,label=r'$\alpha$=%.2f'%popt[1])
print(popt[3])
print(popt[2])
...
#ax.legend(bbox_to_anchor=(0.54, 0.48),handletextpad=0.1,frameon=False,prop={'size':18,'family':f_family})
...
#plt.savefig('C:/python/摩拜单车/draw2/sf_1_b.pdf',bbox_inches='tight')
